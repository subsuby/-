...
.directive('dealerGroupSearchPop', function ($rootScope,$timeout,$http) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'dealer-group-search-pop';	// 변경 <= 팝업ID(폴더,파일명)
...
//							// transcluded DOM을 template에서 ngTransclude directive에 삽입한다
		 ...
		 * #################################################################################### */
			...
			$scope.onComplete = function(){
				if($scope.hidIdG == "" || $scope.hidIdG == null){
					alert("소속단체를 먼저 선택해주세요");
					return false;
				}else{
					$scope.$this.complete(angular.copy($scope.hidIdG));
					$scope.$this.close();
				}
			};
		...
		/* #################################################################################### */
			...
			$scope.searchGrp = function(){
				var danjiFullName = angular.element("input[name=danjiFullNm]").val();
				if($util.isEmpty(danjiFullName)){
					alert("검색어를 입력하세요.");
					return false;
				}else{
					var params={
							danjiFullName: danjiFullName
					};

					var url = BNK_CTX + '/front/common/searchGroup';
					$http({
						url: url
						, method: 'POST'
							, async: false
					...
					, data :JSON.stringify(params)
						...
						//검색한 리스트
						$scope.carGroupList = data.carGroupList;
			...
			$scope.select = function(idx){
				$scope.selectedData = $scope.carGroupList[idx];
				$scope.hidIdG = $scope.selectedData;
			}
		}
	};
});
