 ...
 *
 */
public class PaginatorAjax extends SimpleTagSupport {

	// ******************************************************************
	// *                                           페이지 네비게이터 템플릿 (S)                                     *
	// ******************************************************************

	private String MainSB 		= 	"<div class='paging'>";
    //private String FistB 		= 		"<a class='#$class' click='#$fncName'>#$title</a>";
    private String PrevB 		= 		"<span class='#$class'><a onclick='#$fncName'>#$title</a></span>";
    private String CurB 		= 		"<a class='#$class'>#$title</a>";
    private String OthB 		= 		"<a class='#$class' onclick='#$fncName'>#$title</a>";
    private String NextB 		= 		"<span class='#$class'><a onclick='#$fncName'>#$title</a></span>";
    //private String LastB 		= 		"<a class='#$class' click='#$fncName'>#$title</a>";
    private String MainEB 		= 	"</div>";
    // ******************************************************************
    // *                                           페이지 네비게이터 템플릿 (E)                                     *
    // ******************************************************************

	private String fncName;						// 이동 URL [*페이지 번호는 ##을 Replace]		(설정은 JSTL 속성으로 입력)
    ...
    private int skipSize = 1;					// 이전/다음 선택시 이동할 페이지 갯수				(설정은 JSTL 속성으로 입력)
    ...
    private String prevPageClass = "btn prev";					// 이전 페이지					(설정은 JSTL 속성으로 입력)
    private String curPageClass = "on";					// 현재 페이지					(설정은 JSTL 속성으로 입력)
    private String defaultPageClass = "";				// 다른 페이지					(설정은 JSTL 속성으로 입력)
    private String nextPageClass = "btn next";					// 다음 페이지 					(설정은 JSTL 속성으로 입력)
    ...
    public PaginatorAjax(){}
    public PaginatorAjax(String fncName, int curPage, int totPages){
    ...
    public void doTag() throws JspException {
        ...
        try {
            ...
            // 처음
            //if (goFirstUse) out.write(FistB.replace("#$class", goFirstClass).replace("#$fncName", fncName.replace("##", "1")).replace("#$title", goFirstLabel));

            // 이전
            if (curPage > 1){
            	int moveSize = curPage-skipSize;
            	if (moveSize < 1) moveSize = 1;
            	out.write(PrevB.replace("#$class", prevPageClass).replace("#$fncName", fncName.replace("##", moveSize+"")+"("+moveSize+")").replace("#$title", prevPageLabel));
            }
            ...
            for (int i = pgStart; i < pgEnd; i++) {
                ...
                // 기타 페이지
                else out.write(OthB.replace("#$class", defaultPageClass).replace("#$fncName", fncName.replace("##", i+"")+"("+i+")").replace("#$title", i+""));
            }
            ...
            if (!lastPage){
            	...
            	out.write(NextB.replace("#$class", nextPageClass).replace("#$fncName", fncName.replace("##", moveSize+"")+"("+moveSize+")").replace("#$title", nextPageLabel));
            }

            // 마지막
           // if (goLastUse) out.write(LastB.replace("#$class", goLastClass).replace("#$fncName", fncName.replace("##", totPages+"")).replace("#$title", goLastLabel));
    ...
    public String toString() {
    	...
    	/*boolean lastPage = curPage == totPages;
         ...
         try {
             ...
             // 처음
             //if (goFirstUse && curPage > pgStart) result.append(FistB.replace("#$class", goFirstClass).replace("#$fncName", fncName.replace("##", "1")).replace("#$title", goFirstLabel).replace("#$page", "1"));

             // 이전
             if (curPage > 1){
             	int moveSize = curPage-skipSize;
             	if (moveSize < 1) moveSize = 1;
             	result.append(PrevB.replace("#$class", prevPageClass).replace("#$fncName", fncName.replace("##", moveSize+"")).replace("#$title", prevPageLabel).replace("#$page", moveSize+""));
             }
             ...
             for (int i = pgStart; i < pgEnd; i++) {
                 ...
                 // 기타 페이지
                 else result.append(OthB.replace("#$class", defaultPageClass).replace("#$fncName", fncName.replace("##", i+"")).replace("#$title", i+"").replace("#$page", i+""));
             }
             ...
             if (!lastPage){
             	int moveSize = curPage + skipSize;
             	if (moveSize > totPages) moveSize = totPages;
             	result.append(NextB.replace("#$class", nextPageClass).replace("#$fncName", fncName.replace("##", moveSize+"")).replace("#$title", nextPageLabel).replace("#$page", moveSize+""));
             }

             // 마지막
             //if (goLastUse && curPage < pgEnd-1) result.append(LastB.replace("#$class", goLastClass).replace("#$fncName", fncName.replace("##", totPages+"")).replace("#$title", goLastLabel).replace("#$page", totPages+""));
