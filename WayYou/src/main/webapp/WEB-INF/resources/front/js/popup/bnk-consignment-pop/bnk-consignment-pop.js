...
.directive('consignmentPop', function ($rootScope, $timeout) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'consignment-pop';	// 변경 <= 팝업ID(폴더,파일명)
...
//							// transcluded DOM을 template에서 ngTransclude directive에 삽입한다
		...
		controller: function($scope, $http, $timeout){
			...
			$scope.consignmentClick = function(){
				$scope.$parent.isLogin({
					success: function(){
						if($scope.$parent.sessUserInfo.division == "N"){
							url = BNK_CTX + "/front/my/reserList";	
						}else{
							url = BNK_CTX + "/front/my/reserListDealer";
						}
						location.href = url;
					},
					fail: function(){
