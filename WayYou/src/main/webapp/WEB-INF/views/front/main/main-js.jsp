...
<script>
bnkApp.controller(CTRL_NAME, ['$scope', '$http', '$filter', '$util', '$timeout', '$localstorage', function($scope, $http, $filter, $util, $timeout, $localstorage){
 ...
 * #################################################################################### */
	...
	$scope.oParams.dummy = 18798;	
	$scope.$parent.type = "main";
 ...
 * #################################################################################### */
	...
	$scope.onPopHandler = function(id, data){
		...
		$localstorage.set('oSearch', {oConditions: $scope.oSearch.oConditions, sort: $scope.oSearch.sort});
		location.href = '<c:url value="/front/category/mycar/buyList" />';
	};
 ...
 * #################################################################################### */
	...
	var getList = function(){
		...
		oParams.oConditions = [];								// 퀵검색 조건
		oParams.sortField = 'T1.CREATED_DATE DESC';
...
/* #################################################################################### */
	...
	var init = function(){
		$localstorage.get('oSearch',{}).then(function(oSearch){
			...
			$scope.oSearch.oConditions 	= $util.nvl($scope.oSearch.oConditions, []); 		// 퀵검색 검색조건 담을 배열
	...
	$scope.onClick=function(code,car){
		...
		case 'BTN_SEARCH':
			...
			break;
		case 'BTN_CLOSE':
			//step 1. 쿠키 선택결과 DB 저장
			if ($scope.popCookie){
				$localstorage.set('eventPop', {
					show: 'N'+$filter('date')(new Date(), 'yyyyMMdd')
				});
			}else{
				$localstorage.set('eventPop', { show: '' });
			}
			$scope.showPopup = false;
			break;
		}
	...
	$scope.loadLayerPopup = function(){
		$localstorage.get('eventPop',{}).then(function(eventPop){
			if(eventPop.show == ('N'+$filter('date')(new Date(), 'yyyyMMdd')) ){
				$scope.showPopup = false;
			}else{
