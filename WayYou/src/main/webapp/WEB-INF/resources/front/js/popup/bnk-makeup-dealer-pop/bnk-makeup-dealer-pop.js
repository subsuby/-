...
.directive('makeupDealerPop', function ($rootScope, $timeout, $http, $filter, $localstorage) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'makeup-dealer-pop';		// 변경 <= 팝업ID(폴더,파일명)
...
//							// transcluded DOM을 template에서 ngTransclude directive에 삽입한다
		...
		controller: function($scope){
			...
			$scope.getList = function(){
				/* AJAX 통신 처리 */
				var url = BNK_CTX + "/front/my/makeupDealer/search";
				$http({
					url: url
					, method : 'POST'
					, async: false
			...
			$scope.regist = function(){
				...
				alertify.confirm('메이크업 서비스를 \n신청하시겠습니까?', function(){
					...
					$http({
						method	: 'POST',
						url		: BNK_CTX + '/front/my/makeupDealer/regist',
