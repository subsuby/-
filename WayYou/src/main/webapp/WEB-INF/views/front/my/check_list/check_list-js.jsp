...
bnkApp.controller(CTRL_NAME, ['$scope', '$http', '$util', '$timeout', function($scope, $http, $util, $timeout){
    ...
    $scope.onClick = function(code,items,userName,createdDate, chkListSeq){
    	switch(code){
    	//체크리스트
    	case '3':
    	...
    	//구매비용
    	case '4':
            $scope.oParams.chkListSeq	= chkListSeq;
            $scope.oParams.state		= "mycar1";
    ...
    $scope.getList = function(){
        $http({
            url: "<c:url value='/front/my/checkList/ajax'></c:url>"
            , method: 'POST'
            , async: false
