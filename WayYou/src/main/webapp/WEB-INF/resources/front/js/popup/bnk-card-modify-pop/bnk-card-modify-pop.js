...
.directive('cardModifyPop', function ($rootScope, $timeout) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'card-modify-pop';	// 변경 <= 팝업ID(폴더,파일명)
		...
		controller: function($scope, $http){
			...
			$scope.templateInit = function(){
				...
				$scope.$this.onOpenHandle = function(){
					$scope.open($scope.$parent.cardParams);
				};
			...
			$scope.open = function(param){
				$scope.user = param;
				$scope.user.userId = param.userId;
				$scope.user.userName = param.userName;
				$scope.user.dealerLicenseNo = param.dealerLicenseNo;
				$scope.user.dealerProfileFileSeq = param.dealerProfileFileSeq;
				$scope.user.dealerProfileDesc = param.dealerProfileDesc;
				$scope.user.dealerProfileTel = param.dealerProfileTel;
				$scope.user.dealerDanjiName = param.dealerDanjiName;
				$scope.user.dealerShopName = param.dealerShopName;
			}
			
			$scope.modifyClick = function(){
				var profileDesc = $.trim($("#mList1").val());
				var profileTel = $.trim($("#mList2").val());
				...
				var params={
	                    ...
	                    dealerProfileDesc   : profileDesc,
	                    dealerProfileTel    : profileTel
	                };
				
				$http({
						method:'POST', 
						url:BNK_CTX + '/api/user/modifyNameCard',
						async: false, 
