...
//                    }
             ...
             *
             */
            _b.setupTypePopup = function(){
                var
	                P_BUTTON_PREFIX     = "btn-popup-",
	                POP_ID_PREFIX		= "__pop_up_",
                    sufd                = "default",
                    auto               = "auto",
	                sufp                = "popover",
	                suff                = "full",
	                ...
	                $b					= $('body'),
	                $btns               = $("."+P_BUTTON_PREFIX+"default"
                                               +"," +"."+P_BUTTON_PREFIX+"auto"
	                                           +"," +"."+P_BUTTON_PREFIX+"full"
	                						),
	                $modal				= $(".popupDim"),
	                $wrapBack			= $("#wrap_back"),
	                $wrap				= $("#wrap"),
...
//                    $b.css({"overflow":"hidden", "height":"100%"});
                	...
                	if($popup.hasClass("appraisePop")){
                		$modal.css("z-index",$popup.css("z-index"));
                	}

                    // 기본팝업 높이 조정
                    var ww = $(window).outerWidth();
                    var wh = $(window).outerHeight();
                    var ow_num = $popup.outerWidth()/2;
                    var oh_num = $popup.outerHeight()/2;

                    if($popup.outerHeight() > wh){
                        $popup.css({
                            position : "absolute",
                            top : Math.max(0, 180 + $(window).scrollTop()) + "px",
                            left : "50%",
                            marginLeft : (-ow_num) + "px"
                            /* height : wh-40 + "px",
                            top : "50%",
                            margin : -(wh-40)/2 + "px 0 0" + (-ow_num) + "px" 2017-09-18 by mj-cho */
                        }).add($modal).show();
//                        console.log(wh, $popup.outerHeight(),$popup.outerHeight()/2, $popup.children().find('h1').outerHeight(), $popup.children().find('.paging').outerHeight());
                        /*$popup.find('.control').css({
                            'overflow' : 'hidden',
                            'overflow-y' : 'scroll',
                             height : $popup.outerHeight()-270 + 'px'
                         }); 2017-09-18 by mj-cho*/
                    }else{
                        $popup.css({
                            position : "absolute",
                            top : Math.max(0, 180 + $(window).scrollTop()) + "px",
                            ...
                            marginLeft : (-ow_num) + "px"
                           /* top : "50%",
                            margin : -oh_num + "px 0 0" + (-ow_num) + "px"  2017-09-18 by mj-cho */
                        }).add($modal).show();
                    }
                    if( $popup.hasClass("btnvisitPop") || $popup.hasClass("makeupAdd") ||  $popup.hasClass("sellPop") || $popup.hasClass("carAccident")){
                    		$popup.css('z-index' ,"1201");
                    }
                    /*
                	if($popup.hasClass("popDealerView")){
                		var pos = $(".carListHead.dealerCase").offset().top;
                		$popup.css('top' ,pos+"px");
                	}else if($popup.hasClass("popCostPerson")){
                		var pos = $(".carListHead.ccCase").offset().top;
                		$popup.css('top' ,pos+"px");
                	}else if($popup.hasClass("btnvisitPop")){	// 방문견적
                		var pos = $(".carListHead").offset().top;
                		$popup.css('top' ,pos+"px");
                		$popup.css('z-index' ,"1201");
                	}else if($popup.hasClass("makeupAdd")){	// 메이크업
                		var pos = $(".makeUpHead").offset().top + 180; //2017-09-15
                		$popup.css('top' ,pos+"px");
                		$popup.css('z-index' ,"1201");
                	}else if($popup.hasClass("checkListDetail")){	// 체크리스트
                		var pos = $(".carListHead.checkListCase").offset().top;
                		$popup.css('top' ,pos+"px");
                		$popup.css('z-index' ,"1201");
                	}else if($popup.hasClass("sellPop")){	// 판매차량
                		var pos = $(".dealerInfo").offset().top;
                		$popup.css('top' ,pos+"px");
                		$popup.css('z-index' ,"1201");
                	}else if($popup.hasClass("carAccident")){	// 사고이력
                		var pos = $(".accident").offset().top-456;
                		$popup.css('top' ,pos+"px");
                		$popup.css('z-index' ,"1201");
                	} 2017-09-18 by mj-cho */
...
//                    ItDevice.lockScroll(true);	// 모바일 화면 : 모달레이어 뒤의 화면은 위 css로 스크롤을 막습니다.
                ...
                $b.on(
                    'click',
                    "."+P_BUTTON_PREFIX+sufd+", ."+P_BUTTON_PREFIX+auto+", ."+P_BUTTON_PREFIX+suff+", ."+P_BUTTON_PREFIX+sufp ,
                    function(){
                        var $this = $(this),
                        	opts = $this.data("popOpts"),
                        	target = opts.target,
                        	disYn = opts.display;

                        // disYn이 있고, false일 때
                        if(!!disYn && disYn === "false") return false;
                        ...
                        // auto 팝업일 경우 -> 기본 팝업 오픈
                        if($this.hasClass(P_BUTTON_PREFIX+auto)) autoPop();
	        	...
	        	$b.on(
                    ...
                    '.p-close',
                    function(){
    	        		var $popup = $(this).closest('.p-container');
...
//    	        			$b.css({"overflow":"auto", "height":"auto"});
    	        			...
    	        			if($('.p-container:visible').length === 1){
    	        				$popup.add($modal).hide();
    	        			}else{
    	        				$popup.hide();
    	        				$modal.css("z-index", $('.p-container:visible').css("z-index")-1);
    	        			}
                        ...
                        // 1) auto팝업일 경우
                        if($popup.hasClass("popupAutoWrap")) {
                            $b.css({"overflow":"auto", "height":"auto"});
                            ItDevice.lockScroll(true);
    	        		...
    	        		// 2) 풀팝업일 경우
    	        			...
    	        			var $popupHeader = $popup.find('.popupHeader'),
    		                	$popupContents = $popup.find('.popupContents'),
    		                	$popupBottom = $popup.find('.popupBottom');
    		                ...
    		                .css({"top": "0%"})
    		                    .animate({'top':'100%'}, aniDuration, function(){
    		                    	// 팝업 하단 버튼 사라짐
    		                    	if($popupBottom.size() != 0)$popupBottom.fadeOut(aniDuration/2, function(){$popup.hide();});
    		                    	else $popup.hide();
    	                    });

                            if ($popupContents.length === 0){ // 팝업 컨텐츠가 없다면
