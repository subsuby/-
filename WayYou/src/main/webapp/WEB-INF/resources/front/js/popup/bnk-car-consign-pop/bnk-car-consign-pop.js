...
.directive('carConsignPop', function ($rootScope, $timeout, Upload, $http) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'car-consign-pop';	// 변경 <= 팝업ID(폴더,파일명)
		...
		controller: function($scope, $util, $filter){
			...
			$scope.templateInit = function(){
				...
				$scope.$this.onOpenHandle= function(){
					...
					$scope.res.zipNo		= $scope.$parent.sessUserInfo.zipCode;
					$scope.res.roadAddr		= $scope.$parent.sessUserInfo.addr1;
					$scope.res.addrDtl		= $scope.$parent.sessUserInfo.addr2;
				}
			};
			...
			*************************************/
			...
			$scope.res.reserve	= function(){
				...
				$scope.res.parkZip	 = $util.nvl($scope.res.zipNo, '');
				$scope.res.parkAddr1 = $util.nvl($scope.res.roadAddr, '');
				$scope.res.parkAddr2 = $util.nvl($scope.res.addrDtl, '');
				...
				if($scope.res.parkZip == ""){
					alert("탁송받을 주소를 입력해주세요.");
					return;
				}

				$http({
					method:'POST'
					, url: BNK_CTX + '/api/user/registCarReserve'
					, data: JSON.stringify($scope.res)
				})
				.success(function(data){
					console.log(data);
					if(data.resCd == '00'){
						alertify.alert('탁송신청을 예약하였습니다.');
						$scope.$this.close();
			...
			$scope.onClick = function(code, val){
				...
				case 'BTN_ADDR_CHANGE':
					...
					addrPopup.onCompleteHandle = function(data){
						$scope.res.zipNo		= data.zipNo;
						$scope.res.roadAddr		= data.roadAddr;
						$scope.res.addrDtl		= data.addrDtl;
					};
					break;
				}
			};
