...
.directive('bnkPriceCommon', function ($timeout, $http) {

	var TEMPLATE_ID = 'bnk-price-common';	// 변경 <= 팝업ID(폴더,파일명)
	/**
	 * oParams
	 *
	 *
			...
			$scope.start = function(){
				if(angular.isObject($scope.params)){
					...
					$(".pRoad").animate({width: '100%'}, 800,function(){
							$(".pSection").show();
						$(".pSection span").animate({width: $scope.params.range_width, marginLeft: $scope.params.range_pos },function(){	//노란색 도로 너비, 시작위치
							$(".pSection span em").animate({width: $(".pSection span").width()-22});
						});
					});
					setTimeout(function(){
						$(".pgCar").show();
						$(".pgPos div").animate({width: $scope.params.car_width}, function(){		//차량 너비
