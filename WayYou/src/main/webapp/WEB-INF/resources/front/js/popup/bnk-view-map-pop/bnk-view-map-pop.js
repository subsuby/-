...
.directive('viewMapPop', function ($rootScope, $timeout) {
 ...
 * #################################################################################### */
	...
	var TEMPLATE_ID = 'view-map-pop';	// 변경 <= 팝업ID(폴더,파일명)
...
//							// transcluded DOM을 template에서 ngTransclude directive에 삽입한다
		...
		controller: function($scope, $http, $util){
			...
			$scope.$watch('oParams.user', function(nv){
				console.log('oParams.user change');
				if($util.isNotEmpty(nv)){
					$timeout(function(){
						var shop = shopCodeInfoMap[$scope.oParams.user.shopNo].shop;
						var map = new naver.maps.Map('view', {
						...
						var move = new naver.maps.LatLng(shop.shopLocLat, shop.shopLocLng);
						map.setCenter(move); // 중심 좌표 이동

					}, 50);
				}
			});
			$scope.$watch('oParams.render', function(nv){
