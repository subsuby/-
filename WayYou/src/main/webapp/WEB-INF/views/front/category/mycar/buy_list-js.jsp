 ...
 * #################################################################################### */
	...
	var getList = function(){
		...
		$http({
			method: 'POST',
			url: '<c:url value="/api/car/list/default"/>',
		...
		.success(function(oRes){
			...
			if(oRes.code == "00"){
				...
				// 20170717 jy-seo KFC 요청사항
				// 전체차량 개수가 4만대를 넘게 해달라는 요청사항이있어, 검색조건이 없을 경우 18000대를 카운팅에 하드코딩으로 추가함.
				function isCodeInWords(id, code){	// 검색배열안에 코드가 포함되어있는지 확인
					return oParams.oConditions.findIndex(function(d){return d.id == id && d.code == code}) > -1;
				}
				if($util.isNotEmpty($scope.oParams.dummy) && oRes.data.totListSize > 3000){
					if((isCodeInWords('SEARCH_WORD_COUNTRY', 'A') || isCodeInWords('SEARCH_WORD_COUNTRY', 'B'))
						...
						$scope.oPageInfo.totListSize += (isCodeInWords('SEARCH_WORD_COUNTRY', 'A')? ($scope.oParams.dummy - 3000) : 3000);
					}else{
						$scope.oPageInfo.totListSize += $scope.oParams.dummy;
					} 
				}
				// 20170717 jy-seo KFC 요청사항
